...
WHERE
	...
	(
		select t.PatientSID
		FROM Dflt._1pk_INCLUSION_STEP01_Z AS t
	)
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_INCLUSION_STEP05_C') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_INCLUSION_STEP05_C
	END

CREATE TABLE Dflt._pk_INCLUSION_STEP05_C
(
	PatientSSN VARCHAR(10)
	,PatientSID BIGINT
	,ECEventSID BIGINT
...
INSERT INTO Dflt._pk_INCLUSION_STEP05_C
SELECT DISTINCT
	sp.PatientSSN
	,edl.PatientSID
	,edl.VisitSID
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_INCLUSION_STEP05_D') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_INCLUSION_STEP05_D
	END

CREATE TABLE Dflt._pk_INCLUSION_STEP05_D
(
	PatientSSN VARCHAR(10)
	,PatientSID BIGINT
	,InpSID BIGINT
...
INSERT INTO Dflt._pk_INCLUSION_STEP05_D
SELECT DISTINCT
	sp.PatientSSN
	,inp.PatientSID
	,inp.InpatientSID
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_INCLUSION_STEP05_E') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_INCLUSION_STEP05_E
	END

CREATE TABLE Dflt._pk_INCLUSION_STEP05_E
(
	ECEventSID BIGINT
)
...
INSERT INTO Dflt._pk_INCLUSION_STEP05_E
SELECT DISTINCT
	prev.ECEventSID
FROM
	Dflt._pk_INCLUSION_STEP05_C AS prev INNER JOIN Dflt._pk_INCLUSION_STEP05_D AS ex_inp ON
	(
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_INCLUSION_STEP05_F') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_INCLUSION_STEP05_F
	END

CREATE TABLE Dflt._pk_INCLUSION_STEP05_F
(
	PatientSSN VARCHAR(10)
	,PatientSID BIGINT
	,ECEventSID BIGINT
...
INSERT INTO Dflt._pk_INCLUSION_STEP05_F
SELECT DISTINCT
	prev.PatientSSN
	,prev.PatientSID
	,prev.ECEventSID
	...
	,prev.ECEventEndDateTime
	,prev.TypeOfEvent
FROM Dflt._pk_INCLUSION_STEP05_C AS prev
WHERE
	prev.ECEventSID NOT IN 
	(
		SELECT t.ECEventSID
		FROM Dflt._pk_INCLUSION_STEP05_E AS t
	)
...
SELECT DISTINCT
	...
	,ec.TypeOfEvent
FROM
	Dflt._pk_INCLUSION_STEP05_F AS ec
...
FROM
	...
	(
		...
		ec.ECEventDateTime BETWEEN
			DATEADD(DAY, (SELECT params.Lookback_Length_Days FROM Dflt._pk_ParameterTable AS params), dx.DiagnosisEventDateTime)
			AND
			dx.DiagnosisEventDateTime
	)
...
SELECT
	...
	,prev.ECTypeOfEvent
FROM
	Dflt._pk_EXCLUSION_FINAL AS prev LEFT JOIN Src.Oncology_Oncology_Primary_165_5 AS reg ON 
	(
...
SELECT
	...
	,prev.ECTypeOfEvent
FROM
	Dflt._pk_EXCLUSION_FINAL AS prev LEFT JOIN Src.Inpat_Inpatient AS d_inp ON
		(
...
SELECT
	...
	,prev.ECTypeOfEvent
FROM
	Dflt._pk_EXCLUSION_FINAL AS prev LEFT JOIN Src.Outpat_Workload AS d_owl ON
		(

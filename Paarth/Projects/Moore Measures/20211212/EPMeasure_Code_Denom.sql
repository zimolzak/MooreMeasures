...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_EXCLUSION_STEP03_A') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_EXCLUSION_STEP03_A
	END

CREATE TABLE Dflt._pk_EXCLUSION_STEP03_A
(
	PatientSSN VARCHAR(10)
	,PatientSID BIGINT
	,DiagnosisEventSID BIGINT
...
-- Get all historical inpatient records for patients
INSERT INTO Dflt._pk_EXCLUSION_STEP03_A
SELECT DISTINCT
	inc.PatientSSN
	,inc.PatientSID
	,inc.DiagnosisEventSID
	...
	,'INPATIENT'
FROM
	Dflt._pk_INCLUSION_STEP04_Z AS inc INNER JOIN Src.SPatient_SPatient AS sp ON
	(
		inc.PatientSSN = sp.PatientSSN
		AND
		sp.PatientSID != -1
...
-- Create tables 
IF (OBJECT_ID('Dflt._pk_EXCLUSION_STEP03_Z') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_EXCLUSION_STEP03_Z
	END

CREATE TABLE Dflt._pk_EXCLUSION_STEP03_Z
(
	PatientSSN VARCHAR(10)
	,PatientSID BIGINT
	,DiagnosisEventSID BIGINT
...
-- For each patient, get the earliest historical event from eSTEP 01 PART A
INSERT INTO Dflt._pk_EXCLUSION_STEP03_Z
SELECT DISTINCT
	hx.*
	,DATEDIFF(MONTH, hx.HistoricalEventDateTime, hx.DiagnosisEventDateTime)
FROM
	Dflt._pk_EXCLUSION_STEP03_A AS hx
WHERE
	hx.HistoricalEventDateTime = 
	(
		SELECT TOP 1 t.HistoricalEventDateTime
		FROM Dflt._pk_EXCLUSION_STEP03_A AS t
		WHERE hx.PatientSSN = t.PatientSSN
		ORDER BY t.HistoricalEventDateTime ASC
	)
...
-- Set the emergency care (EC) search period 
DECLARE @STEP04_CareHxThreshold INT

SET @STEP04_CareHxThreshold = (SELECT params.CareHx_Length_Months FROM Dflt._pk_ParameterTable AS params)

PRINT CHAR(13) + CHAR(10) + 'eStep 02 - Minimum Care History Threshold: ' + CAST(@STEP04_CareHxThreshold AS VARCHAR)


-- Create tables 
IF (OBJECT_ID('Dflt._pk_EXCLUSION_STEP04_Z') IS NOT NULL)
	BEGIN
		DROP TABLE Dflt._pk_EXCLUSION_STEP04_Z
	END

CREATE TABLE Dflt._pk_EXCLUSION_STEP04_Z
(
	PatientSSN VARCHAR(10)
	,PatientSID BIGINT
	,DiagnosisEventSID BIGINT
...
-- Select records from the Inclusion steps that have their earliest histoical encounter > the threshold ago.
INSERT INTO Dflt._pk_EXCLUSION_STEP04_Z
SELECT DISTINCT
	prev.PatientSSN
	,prev.PatientSID
	,prev.DiagnosisEventSID
	...
	,prev.TypeOfEvent
FROM
	Dflt._pk_INCLUSION_STEP04_Z AS prev INNER JOIN Dflt._pk_EXCLUSION_STEP03_Z AS ehx ON
	(
		prev.PatientSSN = ehx.PatientSSN
		AND
		prev.DiagnosisEventSID = ehx.DiagnosisEventSID
		AND
		ehx.DxHxDelta > @STEP04_CareHxThreshold
	)
...
-- Numerator
SELECT COUNT(DISTINCT n.PatientSSN) FROM Dflt._pk_EXCLUSION_FINAL AS n WHERE n.DiagnosisTypeOfEvent = 'REGISTRY ENTRY' AND n.ECTypeOfEvent = 'INPATIENT'

-- Denominator
SELECT COUNT(DISTINCT d.PatientSSN) FROM Dflt._pk_EXCLUSION_STEP04_Z AS d

-- Ages
SELECT DISTINCT
	CONCAT('''', fin.PatientSSN, '''')
	,DATEDIFF(YEAR,sp.BirthDateTime, fin.DiagnosisEventDateTime)
FROM
